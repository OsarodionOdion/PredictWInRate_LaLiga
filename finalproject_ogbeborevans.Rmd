...
The response variable for the predictive models will be win rate. In soccer, a draw wins both teams 1 point, and a win earns the winning team 3 points. In calculating the win rate, wins will be calculated as a sum of games won and one-third of games drawn. From this, the win rate can be calculated as a ratio of wins to the number of games in a season (wins+draws+losses).

```{r win_rate}
#calculating win rate for each team.
laliga1617 <- laliga1617 %>%
  mutate(win_rate = 100 * (wins+(0.33*draws))/(wins+draws+losses))
...
```{r scatter_plots}
g <- ggplot(laliga1617)
g + aes(x=goals_scored, y=win_rate)+geom_point()+geom_smooth(method = "lm")
g + aes(x=minutes_per_goal_conceded, y=win_rate)+geom_point()+geom_smooth(method = "lm")
g + aes(x=clean_sheets, y=win_rate)+geom_point()+geom_smooth(method = "lm")
g + aes(x=corners_total, y=win_rate)+geom_point()+geom_smooth(method = "lm")
g + aes(x=average_possession, y=win_rate)+geom_point()+geom_smooth(method = "lm")
g + aes(x=shots_on_target, y=win_rate)+geom_point()+geom_smooth(method = "lm")
g + aes(x=shots_off_target, y=win_rate)+geom_point()+geom_smooth(method = "lm")
g + aes(x=goal_difference_half_time, y=win_rate)+geom_point()+geom_smooth(method = "lm")
g + aes(x=leading_at_half_time, y=win_rate)+geom_point()+geom_smooth(method = "lm")
```

The shape of the confidence interval has been included in the scatter plots to aid with comprehending the variations. Contrary to notion of Spanish football, where high ball possession is perceived as a principal contributor to win rate, it was interesting to see that average possession and total corner kicks were among variables with the thickest band. 
...
```{r null_full}
nullModel <- lm(win_rate ~ 1, data = laliga1617)
fullModel <- lm(win_rate ~ goals_scored + minutes_per_goal_conceded + clean_sheets + corners_total + average_possession +
                  shots_on_target + shots_off_target + goal_difference_half_time + leading_at_half_time, data = laliga1617)
